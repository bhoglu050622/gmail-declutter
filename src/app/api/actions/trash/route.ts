...
export async function POST(request: Request) {
  ...
  const { scanId, categories } = await request.json();
  ...
  const result = await trashMessages(session.email, session.appPassword, messageIds);

  if (result.success > 0) markEmailsActioned(messageIds, scanId, "TRASHED");
